...
function renderClipsGrid(clips) {
  ...
  clips.forEach((clip) => {
    ...
    // Solo tiene sentido ofrecer el ajuste guardado en clips aún sin estabilizar
    // ("convertido"): un clip que ya viene de la carpeta "estabilizado" es un vídeo YA
    // procesado — aplicarle además el prefiltro de vid.stab en el montaje lo estabilizaría
    // dos veces.
    if (clip.stabilize_draft && clip.source === "convertido") {
      const badge = document.createElement("div");
      badge.className = "clip-stab-badge";
      badge.textContent = "🩹 ajuste de estabilización guardado";
...
function addClipToTimeline(path, atIndex = null) {
  ...
  const item = {
    ...
    out: clip.duration || 0,
    title: null,
    stabilize: (clip.stabilize_draft && clip.source === "convertido") ? { ...clip.stabilize_draft } : null,
  };
